# Kiteline — Professional commercial kitchen SOP pack

**Product:** Kiteline  
**Audience:** Hotels, restaurants, catering, schools, and multi-site production kitchens  
**Version:** 1.1
**Effective date:** 31 August 2026  

This pack is **commercial kitchen only**. It is not a home-cooking guide and it is not the Vedanta vegetarian SOP pack.

## Procedures

| SOP ID | Title | Station |
|--------|--------|---------|
| CK-00 | Brigade & kitchen standards | Hygiene |
| CK-01 | Opening checks | Hygiene |
| CK-02 | Goods-in & deliveries | Stores |
| CK-03 | Storage, FIFO & labelling | Stores |
| CK-04 | Mise en place | Line |
| CK-05 | Cooking temperatures & HACCP | Line |
| CK-06 | Allergen control | Line |
| CK-07 | Pass & service | Service |
| CK-08 | Cleaning & COSHH | Hygiene |
| CK-09 | Closing checks | Close-down |
| CK-10 | Manual handling | Safety |
| CK-11 | Knives & equipment | Safety |
| CK-12 | Raw & ready-to-eat separation | Safety |

Each SOP has a **captioned animated briefing with optional British-English narration** (about one minute) in the pocket app. Completion is stored locally for offline use.

**Guideline B — Recipe not found** is the stop rule when a controlled recipe card is missing. It is a guideline, not a recipe. See `GUIDELINE-B-RECIPE-NOT-FOUND.md`.

## Pocket app

Open in this repo: [`kitchen-sop/`](../../../kitchen-sop/).  
`https://kiteline.uk/kitchen-sop/` is **not live yet** (404 until kitline1 is updated).

Source: [`kiteline-kitchen-sop/`](../../../kiteline-kitchen-sop/)  
Kiteline drop-in: [`kiteline-uk-dropin/`](../../../kiteline-uk-dropin/)

## House rules (post on the pass)

1. Professional commercial kitchen only.  
2. Uniform, hair restrained, closed shoes, no jewellery on the line.  
3. Hand-wash at the dedicated sink.  
4. Raw and ready-to-eat stay separate.  
5. Probe and record — do not guess temperatures.  
6. Guideline B — if the recipe card is not found, stop. Do not cook from memory. 86 the dish and tell the chef.  
7. Hot hold 63 °C+. Cold hold 8 °C or below (5 °C target). Freezer −18 °C.  
8. Label internal containers with name, date and use-by. PPDS food needs the food name and full ingredients list with regulated allergens emphasised.  
9. 14 UK/EU allergens — if unsure, do not send.  
10. FIFO. Quarantine bad deliveries.  
11. Stop the line and tell the chef if a CCP or allergen fails.

## Training sign-off

Print `TRAINING-SIGN-OFF.md`. Every starter signs CK-00, CK-05, and CK-06.

## Document control

| Field | Value |
|-------|--------|
| Owner | Head chef / kitchen manager |
| Review | 31 August 2027, or earlier after an incident, legal change, menu/allergen change or process change |
| Brand | Kiteline |
