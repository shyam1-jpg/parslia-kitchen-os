#!/usr/bin/env python3
"""Sort company emails into Outlook folders.

Used by tests, the HTML preview, and the rules cheat-sheet.
The Windows one-click script files the same way in the live Hotmail inbox
(outlook.live.com) after you sign in.
"""
...
def rules_cheat_sheet(companies: list[Company] | None = None) -> str:
    companies = companies if companies is not None else load_companies()
    lines = [
        "Outlook rules to add (only if the one-click script cannot run)",
        "============================================================",
        "",
        "Best path: double-click FILE-ALL-EMAIL-FOLDERS.bat and sign in.",
        "That files ALL company mail in your Hotmail (outlook.live.com).",
        "",
        "Manual path: https://outlook.live.com/mail/ → Settings → Mail → Rules",
        "",
        f"Create a folder named {PARENT_FOLDER} under Inbox, then one folder per company.",
        "Outlook on the web allows about 64 rules, so add the companies you see most.",
        "",
    ]
    for company in companies[:MAX_INBOX_RULES]:
...
def write_generated_files(
    ...
    summary = {
        ...
        "source_folder_ids": load_source_folder_ids(),
    }
