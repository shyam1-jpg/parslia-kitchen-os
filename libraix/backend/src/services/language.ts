/**
 * Lightweight language detection from script + common words.
 * Used so Libraix replies (and TTS) match the user's language, like ChatGPT.
 */

export type DetectedLanguage = {
  code: string; // BCP-47-ish: en, hi, ta, te, ml, kn, bn, gu, pa, mr, ur, ar, zh, ja, ko, es, fr, de, pt, it, ru, …
  name: string;
  speechLocale: string; // for Web Speech API
...
/** Detect language from user text (Unicode scripts + a few word heuristics). */
export function detectLanguage(text: string): DetectedLanguage {
  ...
  for (const ch of sample) {
    ...
    if (cp >= 0x0900 && cp <= 0x097f) bump("hi"); // Devanagari (Hindi/Marathi/etc.)
    ...
    else if (cp >= 0x0600 && cp <= 0x06ff) bump("ar"); // Arabic / Urdu script
  ...
  // Prefer Urdu when Arabic script + Urdu-specific chars / words
  if ((counts.ar ?? 0) > 0 && /[\u0679\u0688\u0691\u06be\u06d2]|(\b(میں|ہے|کیا|اور)\b)/.test(sample)) {
    bump("ur", 8);
  }
  // Marathi often Devanagari — boost if distinctive words
  ...
  if (/\b(и|это|что|привет|спасибо)\b/i.test(sample)) bump("ru", 4);
  if (/\b(ve|bir|için|merhaba|teşekkür)\b/i.test(sample)) bump("tr", 4);
  if (/\b(yang|dan|untuk|terima|apa)\b/i.test(lower)) bump("id", 4);
  ...
  if (!ranked.length) return result("en", "low");
  ...
  // Devanagari default Hindi unless Marathi won
  if (top === "hi" || top === "mr") return result(top, confidence);
  if (top === "ar" && (counts.ur ?? 0) > counts.ar!) return result("ur", confidence);
  return result(top, confidence);
}

export function languageReplyInstruction(lang: DetectedLanguage): string {
  if (lang.code === "en" && lang.confidence === "low") {
    return "Reply in the same language the user wrote in. If unclear, use clear English.";
  }
  return (
    `The user is writing in ${lang.name} (${lang.code}). ` +
    `Reply entirely in ${lang.name}, naturally and fluently — like a native speaker. ` +
    `Keep Markdown structure, but all prose must be in ${lang.name}. ` +
    `Do not switch to English unless the user mixes English or asks for English.`
  );
}

export const SUPPORTED_REPLY_LANGUAGES = Object.keys(NAMES).map((code) => ({
  code,
  name: NAMES[code],
  speechLocale: SPEECH_LOCALES[code] ?? code,
}));
