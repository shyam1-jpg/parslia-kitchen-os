...
export default function SignIn() {
  ...
  useEffect(() => {
    ...
    api<{ microsoft: boolean; dev: boolean }>("/auth/providers").then(p => { setProviders(p); if (p.dev) api<{ items: U[] }>("/auth/users").then(r => setUsers(r.items)).catch(() => {}); }).catch(() => setErr("Cannot reach the API."));
  }, [signInWithToken]);
  ...
  return (
    <div className="signin">
      ...
      {providers?.dev && (<>
        ...
        <div className="frow" style={{ maxWidth: 420, marginTop: 18 }}><input placeholder="or type an email" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && go(email)} /><button className="btn primary" disabled={!email || busy} onClick={() => go(email)}>Sign in</button></div>
      </>)}
      {providers && !providers.microsoft && !providers.dev && <div className="note">No sign-in method is configured. Set the MS_* variables on the API.</div>}
