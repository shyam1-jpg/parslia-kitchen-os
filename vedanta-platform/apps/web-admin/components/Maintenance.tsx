...
export function ReportFault({ room, onClose, onDone }: { room?: string; onClose: () => void; onDone: (msg: string) => void }) {
  const [f, setF] = useState({ title: "", description: "", room: room ?? "", location: "", priority: "NORMAL", takes_room_out: false }); const [err, setErr] = useState<string | null>(null);
  const submit = async () => { try { const r = await api<{ number: number }>("/v1/maintenance", { method: "POST", body: JSON.stringify({ ...f, room: f.room || undefined, location: f.location || undefined }) }); onDone(`Reported as M-${r.number}${f.takes_room_out && f.room ? ` — room ${f.room} is out of order` : ""}`); } catch (e) { setErr(e instanceof ApiError ? e.problem.detail : "Could not report"); } };
  return (
    <div className="modal-backdrop" onClick={onClose}><div className="modal" onClick={e => e.stopPropagation()} role="dialog">
      ...
      <div className="fgrid">
        ...
        <label>Room<input value={f.room} onChange={e => setF({ ...f, room: e.target.value })} placeholder="e.g. 110" /></label>
        <label>Or location<input value={f.location} onChange={e => setF({ ...f, location: e.target.value })} placeholder="Kitchen, lakeside path…" /></label>
      ...
      <div className="actions"><button className="btn" onClick={onClose}>Cancel</button><button className="btn primary" disabled={!f.title.trim()} onClick={submit}>Report</button></div>
    </div></div>);
}
    ...
    <>
      ...
      <div className="hkgrid">{items.map(t => (
        <div key={t.id} className={"hk " + (t.priority === "SAFETY" || t.priority === "URGENT" ? "VACANT_DIRTY" : t.status === "IN_PROGRESS" ? "CLEANING" : t.status === "DONE" ? "INSPECTED" : "VACANT_CLEAN")}>
          ...
          <div className="m" style={{ marginTop: 4 }}>{ST[t.status]} · reported by {t.reported_by ?? "—"} {new Date(t.created_at).toLocaleDateString("en-GB")}{t.takes_room_out ? " · room out of order" : ""}</div>
