...
const FROM = process.env.MAIL_FROM ?? "Vedanta Oway Retreat <bookings@thevedanta.org>";
...
export function renderFormLink(g: any, url: string, signer: string) {
  return { subject: `${g.name} — guest list for your stay at Vedanta Oway Retreat`,
...
With warm regards,
${signer}
Vedanta Oway Retreat · The Vedanta Way Ltd · https://www.thevedanta.org/` };
}

export function renderConfirmation(g: any, signer: string) {
  const v = bookingValue(g);
  return { subject: `Booking confirmed — ${g.name} at Vedanta Oway Retreat`,
    body: `Dear ${g.contact_name ?? g.organisation ?? "organiser"},
